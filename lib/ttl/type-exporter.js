 ...
 */
export async function exportAsync(file) {
  const tmpFile = `${file}.tmp`;
  const fileContent = await fs.readFile('/config/type-export.json');
...
async function appendBatch(file, query, offset = 0) {
  ...
  try {
    ...
    // We need to post it as form data for support of large-query strings.
    // This is the quickest I could come up with; given demo
    // TODO: refactor later to mu-auth-sudo
    ...
    await fs.appendFile(file, resultText + '\n');
  } catch (e) {
    console.log(e);
    throw e;
  }
...
function prefixStatements(prefixes) {
  return Object.keys(prefixes).map(
    (prefix) => `PREFIX ${prefix}: <${prefixes[prefix]}>\n`,
  );
}
