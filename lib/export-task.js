  ...
  async perform() {
    ...
    try {
      ...
      if (existsSync(env.CSV_EXPORT_SPARQL_FILE)) {
        ...
        console.warn(
          `Not creating a CSV export: file ${env.CSV_EXPORT_SPARQL_FILE} does not exist or no access to it`,
        );
...
export async function insertNewTask() {
  ...
  await updateSudo(createJobQuery, {}, env.sparqlConnectionOptions);
  ...
  await updateSudo(createTaskQuery, {}, env.sparqlConnectionOptions);
...
export async function finishTask(task, files = [], failed = false) {
  ...
  if (files.length) {
    ...
    await updateSudo(insertContainerQuery, {}, env.sparqlConnectionOptions);
  }
  ...
  await updateSudo(updateTaskQuery, {}, env.sparqlConnectionOptions);
  ...
  await updateSudo(updateJobQuery, {}, env.sparqlConnectionOptions);
}
...
export async function cleanup() {
  ...
  let result = await querySudo(
    selectCleanupTasksQuery,
    {},
    env.sparqlConnectionOptions,
  );
